...
package com.lovping.demo;

import android.app.Activity;
import android.os.Bundle;
import android.widget.TextView;
import android.util.Log;
import com.lovping.curl.Curl;
import com.lovping.curl.Ping;
...
public class Demo extends Activity {
	...
	public class Data extends Curl {
		...
		public String getURL(String url) {
			...
			// CURLOPT_WRITEFUNCTION will have header too if CURLOPT_HEADER is
			// true
			...
			curl_setopt(curl, CURLOPT_DEBUGFUNCTION, new Curl.Debug() {
				public int callback(int type, byte[] ptr) {
					...
					if (type == CURLINFO_TEXT) {
						Log.d("CURL-J-DEBUG", CURLINFO[type] + ": "
								+ new String(ptr));
					}
					return 0;
				}
			...
			curl_setopt(curl, CURLOPT_PROGRESSFUNCTION, new Curl.Progress() {
				public int callback(double dltotal, double dlnow,
